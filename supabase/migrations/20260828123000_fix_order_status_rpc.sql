...
begin
  ...
  if v_previous_status <> v_status then
    ...
    update public.orders o
      set status = v_status,
          reversal_reason = case when v_status in ('cancelled', 'returned') then v_reason else o.reversal_reason end,
          refund_amount = case when v_status in ('cancelled', 'returned') then v_refund_amount else o.refund_amount end,
          reversal_note = case when v_status in ('cancelled', 'returned') then v_note else o.reversal_note end,
          reversed_at = case when v_status in ('cancelled', 'returned') then now() else o.reversed_at end
    where o.id = p_order_id;

    insert into public.order_events (
      order_id, previous_status, new_status, reason, refund_amount, note
    ) values (
      p_order_id, v_previous_status, v_status, v_reason, v_refund_amount, v_note
    );
  end if;
...
revoke all on function public.update_order_status_with_inventory(uuid,text,text,numeric,text)
  from public, anon, authenticated;
grant execute on function public.update_order_status_with_inventory(uuid,text,text,numeric,text)
  to service_role;
